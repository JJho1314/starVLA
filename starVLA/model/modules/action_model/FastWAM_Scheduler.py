...
"""Continuous-time Flow-Matching scheduler used by FastWAM's action expert.

Ported verbatim from FastWAM `fastwam/models/wan22/schedulers/scheduler_continuous.py`.
No external project deps; only torch. Used by:
  - training: sample_training_t -> add_noise -> training_target (= noise - x_clean)
  - inference: build_inference_schedule -> step (Euler-style flow-matching update)

Timestep convention: this scheduler returns timesteps in the range
[0, num_train_timesteps] (default [0, 1000]), not normalized to [0, 1].
The internal `sigma = t / num_train_timesteps` is what plays the role of the
flow-matching mixing coefficient: `x_t = (1-sigma)*x_clean + sigma*noise`.

Mirrors FastWAM's `WanContinuousFlowMatchScheduler` so action-side training
math is identical to the official repo.
"""
...
class WanContinuousFlowMatchScheduler:
    ...
    def _phi(u: torch.Tensor, shift: float) -> torch.Tensor:
        """Shift transform: pushes u in [0,1] toward more "noise" (high t) when shift>1."""
    ...
    def training_weight(self, timestep: torch.Tensor) -> torch.Tensor:
        """Per-sample loss reweighting (Gaussian-shaped around mid-timestep)."""
    ...
    def add_noise(
        ...
        """x_t = (1 - sigma) * x_clean + sigma * noise, where sigma = t / num_train_timesteps."""
    ...
    def training_target(
        ...
        """Velocity target for flow-matching: v = noise - x_clean (independent of t)."""
        del timestep
        return noise - sample
    ...
    def step(
        ...
        """Euler step in flow-matching: x_{next} = x + v_pred * delta_sigma."""
