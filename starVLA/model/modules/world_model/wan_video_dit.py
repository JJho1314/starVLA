"""Vendored from FastWAM `fastwam/models/wan22/wan_video_dit.py` (verbatim copy).
Imports adapted to starVLA: stdlib logging + inline gradient_checkpoint_forward.
This is the bf16-clean WanVideoDiT used by FastWAM training. Replaces diffusers'
`WanTransformer3DModel` whose forward casts to fp32 5-6× per block (~14 GiB
extra activation at bs=16 across 30 blocks)."""
...
def gradient_checkpoint_forward(model, use_gradient_checkpointing, *args, **kwargs):
    if use_gradient_checkpointing:
        return torch.utils.checkpoint.checkpoint(
            _create_custom_forward(model), *args, **kwargs, use_reentrant=False
        )
