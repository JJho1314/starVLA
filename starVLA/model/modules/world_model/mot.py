"""Vendored from FastWAM `fastwam/models/wan22/mot.py` (verbatim copy).
Imports adapted to starVLA: stdlib logging.
Owns the joint multi-expert attention loop. Each layer concatenates Q/K/V across
experts (video + action), runs one shared SDPA, then per-expert post-attention
(o-proj + cross-attn + FFN). FastWAM's reference implementation; replaces
starVLA's MoTAttnProcessor side-channel hack."""
