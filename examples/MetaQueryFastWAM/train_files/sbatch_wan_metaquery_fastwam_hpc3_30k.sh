...
#
# Aligned with `run_libero_train_wanfastwam_fwalign_joint.sh` (the current
# canonical 8-GPU recipe). Differences vs that script are documented inline.
#
# Compute budget (matches fwalign_joint):
...
# Silence torchvision_av deprecation noise (matches fwalign_joint).
...
# ---------------------------------------------------------------------------
# Path defaults (HPC3 → LFT-W02 fallback — same dual-detection as fwalign_joint)
# ---------------------------------------------------------------------------
...
# Same batch sizing as fwalign / fwalign_joint baseline (verified 8 × H100-80G).
...
echo "MoT attention:       ${MOT_MODE}"
echo "Num video frames:    ${NUM_VIDEO_FRAMES}"
echo "Base WM (diffusers): ${BASE_WM}"
echo "Base VLM:            ${BASE_VLM}"
echo "VLM attn impl:       ${VLM_ATTN}"
...
echo "Val interval:        ${VAL_INTERVAL} (× ${VAL_NUM_BATCHES} batches)"
...
# ---------------------------------------------------------------------------
# Launch (CLI structure matches run_libero_train_wanfastwam_fwalign_joint.sh)
# ---------------------------------------------------------------------------
"${ACCELERATE_BIN}" launch \
    ...
    --framework.fastwam.mot_attention_mode "${MOT_MODE}" \
    --framework.fastwam.num_video_frames "${NUM_VIDEO_FRAMES}" \
