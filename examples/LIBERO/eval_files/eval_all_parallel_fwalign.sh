#!/bin/bash
# Massively parallel LIBERO eval:
# - 2 policy servers (1 per GPU)
# - 4 suites × N_WORKERS_PER_SUITE clients = (4*N) eval processes in parallel
# - Each worker handles a task_id slice; servers shared via round-robin port
set -u
...
N_WORKERS_PER_SUITE=5
TASKS_PER_WORKER=2
SUITES=(libero_spatial libero_object libero_goal libero_10)

BATCH_SIZE=${BATCH_SIZE:-10}      # 20 workers / 2 servers = 10 per server
BATCH_WAIT_MS=${BATCH_WAIT_MS:-20}

start_server() {
  local gpu="$1" port="$2"
  echo "[$(date +%H:%M:%S)] launching server on GPU ${gpu} port ${port} (batch_size=${BATCH_SIZE} wait=${BATCH_WAIT_MS}ms)"
...
# 2) Launch all eval workers in parallel
...
for suite in "${SUITES[@]}"; do
  ...
  for w in $(seq 0 $((N_WORKERS_PER_SUITE-1))); do
    ...
    # Round-robin assign port (server)
    port=${PORTS[$((client_idx % 2))]}
...
echo "Total workers launched: ${#WORKER_PIDS[@]}"
...
echo "[$(date +%H:%M:%S)] all eval workers finished"

# 4) Aggregate summaries per suite
echo "=== AGGREGATE ==="
for suite in "${SUITES[@]}"; do
  out_root="${RUN_DIR}/results/${suite}/${folder}"
