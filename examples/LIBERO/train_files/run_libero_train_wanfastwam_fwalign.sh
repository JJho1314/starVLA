#!/usr/bin/env bash
# WanFastWAM LIBERO training with FastWAM-aligned IO modules.
#
# Differs from `run_libero_train_wanfastwam_v3par3_repro.sh`:
#   - config_yaml = starvla_wanfastwam_libero_fwalign.yaml
#     (uses Wan2_fastwam.WanVideoBackboneFastWAM with DiffSynth-Studio
#      .safetensors VAE / T5 / scheduler instead of HF Diffusers)
#   - Requires:
#       STARVLA_FASTWAM_REPO_PATH      → path to a github.com/yuantianyuan01/FastWAM clone
#                                        (must contain src/fastwam/)
#       STARVLA_FASTWAM_CHECKPOINTS_ROOT → path containing
#                                        DiffSynth-Studio/Wan-Series-Converted-Safetensors/
#                                          {Wan2.2_VAE,models_t5_umt5-xxl-enc-bf16}.safetensors
#                                        AND Wan-AI/Wan2.1-T2V-1.3B/google/umt5-xxl/
#
# Hypothesis: FastWAM-official libero_10 = 95.2% but our HF-stack v3par3-repro = 90% on
# the same suite. The 4.8% gap correlates with HF-vs-DiffSynth T5 cosine drop
# (0.47–0.69 — see scripts/compare_vae_t5_hf_vs_fastwam.py). Re-training with
# this yaml aligns the T5 stack and should close that gap.
#
# Typical HPC3 launch (after `srun` allocation):
#   tmux new -s train 'bash examples/LIBERO/train_files/run_libero_train_wanfastwam_fwalign.sh'
#
# Useful overrides:
#   NUM_GPUS=8 bash examples/LIBERO/train_files/run_libero_train_wanfastwam_fwalign.sh
...
config_yaml="${CONFIG_YAML:-./examples/LIBERO/train_files/starvla_wanfastwam_libero_fwalign.yaml}"
data_mix="${DATA_MIX:-libero_all_fastwam}"
run_root_dir="${RUN_ROOT_DIR:-./playground/Checkpoints}"
run_id="${RUN_ID:-$(date +%Y%m%d_%H%M%S)_libero_${Framework_name}_fwalign}"
