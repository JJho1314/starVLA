#!/usr/bin/env bash
# Launch fwalign-JOINT training on Olabots (single-node, 8x H100 80GB).
#
# Sibling of `launch_olabots_fwalign.sh` — same paths / env / NUM_GPUS=8 /
# BATCH_SIZE=8 / GRAD_ACCUM=2 (EFF_BS=128). Only swap is calling
# `run_libero_train_wanfastwam_fwalign_joint.sh` instead of the fwalign sibling,
# which forces `framework.fastwam.mot_attention_mode: joint`.
#
# Recommended usage:
#   ssh olabots
#   tmux new -s fwjoint
#   bash examples/LIBERO/train_files/launch_olabots_fwalign_joint.sh
#   # Ctrl+B D to detach. tmux attach -t fwjoint to reconnect.
#
# Override knobs (env vars):
#   NUM_GPUS=4               — use fewer GPUs (default: 8)
#   BATCH_SIZE=8 GRAD_ACCUM=2 — effective batch knobs (default 8x2x8 = 128)
#   MAX_STEPS=21700          — total training steps (same as fwalign default)
#   WANDB_API_KEY=... export before calling, or set in ~/.bashrc

set -euo pipefail

# === Olabots layout (same as launch_olabots_fwalign.sh) ===
...
# GPU / batch sizing — identical to fwalign for a clean 97.05-vs-joint comparison
...
echo "[ok] all required paths exist (joint training)"
...
echo "  mot_attention_mode:    joint (forced via yaml + CLI override)"
echo

cd "$STARVLA_DIR"
exec bash examples/LIBERO/train_files/run_libero_train_wanfastwam_fwalign_joint.sh "$@"
