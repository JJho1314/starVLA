...
run_libero_eval() {
    ...
    launch_task_on_pane() {
        ...
        tmux send-keys -t $SESSION_NAME:$pane_info "cd $ROOT_DIR && export EXP_NAME=$EXP_NAME PYTHONPATH=$PYTHONPATH LIBERO_HOME=$LIBERO_HOME LIBERO_CONFIG_PATH=$LIBERO_CONFIG_PATH MUJOCO_GL=$MUJOCO_GL PYOPENGL_PLATFORM=$PYOPENGL_PLATFORM DIFFSYNTH_MODEL_BASE_PATH=$DIFFSYNTH_MODEL_BASE_PATH && \
            STATUS_FILE='$status_file' LOG_FILE='$log_file' RESULT_FILE='$result_file' && \
            CUDA_VISIBLE_DEVICES=$gpu_id "${EVAL_PYTHON:-python}" examples/LIBERO/fastwam_eval/eval_libero_single.py \
    ...
    while true; do
        ...
        if [ $((current_time - last_status_time)) -ge $status_interval ]; then
            ...
            # Per-task trial progress (from progress files written by eval_libero_single.py)
            TASK_PROGRESS_DIR="$OUTPUT_DIR/task_progress"
            if [ -d "$TASK_PROGRESS_DIR" ]; then
                echo "[$(date '+%Y-%m-%d %H:%M:%S')] === Per-Task Trial Progress ==="
                for pf in "$TASK_PROGRESS_DIR"/*.progress; do
                    [ -f "$pf" ] || continue
                    task_label=$(basename "$pf" .progress)
                    progress_info=$(cat "$pf" 2>/dev/null)
                    trials_done=$(echo "$progress_info" | cut -d'|' -f1)
                    sr_info=$(echo "$progress_info" | cut -d'|' -f2)
                    echo "[$(date '+%Y-%m-%d %H:%M:%S')]   $task_label: $trials_done trials ($sr_info)"
                done
                echo "[$(date '+%Y-%m-%d %H:%M:%S')] =================="
            fi
    ...
    "${EVAL_PYTHON:-python}" examples/LIBERO/fastwam_eval/summarize_results.py --output_dir="$OUTPUT_DIR"
}
