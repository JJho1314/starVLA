...
project_root = Path(__file__).resolve().parent  # examples/LIBERO/fastwam_eval/
...
def _write_progress(cfg: DictConfig, trial_idx: int, num_trials: int, successes: int) -> None:
    output_dir = Path(cfg.EVALUATION.output_dir)
    progress_dir = output_dir / "task_progress"
    progress_dir.mkdir(parents=True, exist_ok=True)
    suite = cfg.EVALUATION.task_suite_name
    task_id = cfg.EVALUATION.task_id
    progress_file = progress_dir / f"{suite}_task{task_id}.progress"
    progress_file.write_text(
        f"{trial_idx + 1}/{num_trials}|successes={successes}"
    )
...
def run_single_task(
    ...
    *,
    ...
    for trial_idx in range(num_trials):
        ...
        _write_progress(cfg, trial_idx, num_trials, results["successes"])
        if visualize_future_video:
